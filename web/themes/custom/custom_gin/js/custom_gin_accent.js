...
(($, Drupal, drupalSettings) => {
  Drupal.behaviors.customGinAccent = {
    attach: function attach() {
      const path = drupalSettings.path.currentPath;

      // Set focus color.
      Drupal.behaviors.customGinAccent.setFocusColor();
      ...
      if ((
        ...
        Drupal.behaviors.customGinAccent.setAccentColor();

        const accentColorPreset = drupalSettings.custom_gin.preset_accent_color;

        if (accentColorPreset === 'custom') {
          const accentColorSetting = drupalSettings.custom_gin.accent_color;
    ...
    setAccentColor: function setAccentColor(preset = null, color = null) {
      const accentColorPreset = preset != null ? preset : drupalSettings.custom_gin.preset_accent_color;

      // Clear things up if not custom color is set.
      if (accentColorPreset === 'custom') {
        // Set preset color.
        $('body').attr('data-gin-accent', preset);
        Drupal.behaviors.customGinAccent.setCustomAccentColor('custom', color);
      } else {
        // Set preset color.
    ...
    setCustomAccentColor: function setCustomAccentColor(preset = null, color = null) {
      const accentColorSetting = color != null ? color : drupalSettings.custom_gin.accent_color;
      ...
      if (preset === 'custom') {
        ...
        : drupalSettings.custom_gin.darkmode;
        ...
        if (accentColor) {
          ...
          const styles = `<style class="gin-custom-colors">\
            body:not(.gin-inactive) {\n\
              ...
              --colorGinPrimaryHover: ${Drupal.behaviors.customGinAccent.shadeColor(accentColor, -10)};\n\
              --colorGinPrimaryActive: ${Drupal.behaviors.customGinAccent.shadeColor(accentColor, -15)};\n\
    ...
    setFocusColor: function setFocusColor(preset = null, color = null) {
      const focusColorPreset = preset != null ? preset : drupalSettings.custom_gin.preset_focus_color;
      const focusColorSetting = color != null ? color : drupalSettings.custom_gin.focus_color;

      // First clear things up.
      Drupal.behaviors.customGinAccent.clearFocusColor();
